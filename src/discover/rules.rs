...
pub const RULES: &[RtkRule] = &[
    ...
    RtkRule {
        pattern: r"^(uv\s+run\s+)?(python3?\s+-m\s+)?mypy(\s|$)",
        rtk_cmd: "rtk mypy",
        rewrite_prefixes: &[
    ...
    RtkRule {
        pattern: r"^(uv\s+run\s+)?ruff\s+(check|format)",
        rtk_cmd: "rtk ruff",
        rewrite_prefixes: &["uv run ruff", "ruff"],
        category: "Python",
        savings_pct: 80.0,
    ...
    RtkRule {
        pattern: r"^(uv\s+run\s+)?(python[0-9.]*\s+-m\s+)?pytest(\s|$)",
        rtk_cmd: "rtk pytest",
        rewrite_prefixes: &[
